...
contains
  ...
  subroutine mp_pumas_pre_run(ncol, nlev, nlevp1, ntrac, tgrs, prsl, prsi, qgrs, &
       ntqv, ntcw, ntiw, ntlnc, ntinc, ntrnc, ntsnc, ntrw, ntsw, ntgl, ntgnc, &
...
!- IH
    ...
    integer, intent(in) :: ntqv, ntcw, ntiw, ntlnc, ntinc, ntrnc, ntsnc, ntrw, ntsw, ntgl, ntgnc
!+ IH
...
!+ IH: Additional required vars
    ...
    real(kind_phys), dimension(:,:),   intent(in) :: clouds1
...
!    micro_x(i,micro_nlev) = x(i,1): x at the surface.
        ...
        do l = 1,micro_dust_nbins
          micro_rndst(i,k,l) = 2.e-7_kind_phys
          micro_nacon(i,k,l) = 0._kind_phys
        end do
      end do
...
! Default in the MG3 scheme, yet 'cld_shoc(i,rk)' should be zero so tx1 is basically 'clcn_i(i,rk)'
!        if ( do_shoc ) then
...
!        end if
!
!+++ test#1: Inputting the up-to-the-point cloud fraction -> the result does not change, maybe because 'qgrs(i,rk,ntclamt)' is essentially 'clcn_i(i,rk)'?
!        tx1 = qgrs(i,rk,ntclamt)
!--- test#1
!+++ test#2: Latest try - using the cloud fraction diagnosed by the radiation scheme (from the previous time step)
        tx1 = clouds1(i,rk)
!--- test#2
        ...
        if ( tgrs(i,rk) .gt. tice_i ) then
          ...
          tx2 = tx * (tice_i - tgrs(i,rk)) / (tice_i - tice_all)
...
!  (1992) has the unit of L-1.
    ...
    ! (ad-hoc) immersion nucleation in mixed-phase clouds (-37 C < T < 0 C) following Gjelsvik et al. (2025)
    ! When using the INP of Gjelsvik et al. (2025), the calling of 'immersion_freezing' in micro_pumas_v1.F90 may need to be commentted out
    !do i = 1, micro_ncol
    !  do k = 1, micro_nlev
    !    rk = micro_nlev - k + 1
    !    if ( tgrs(i,rk) .lt. 273.15_kind_phys .and. tgrs(i,rk) .gt. 236.15_kind_phys .and. qgrs(i,rk,ntcw) .gt. 1.e-12_kind_phys ) then
    !      micro_frzimm(i,k) = 1.e-3_kind_phys * exp( -10.105_kind_phys - 0.342_kind_phys*tgrs(i,rk) ) ! L-1 -> cm-3
    !    else
    !      micro_frzimm(i,k) = 0._kind_phys
    !    end if
    !  end do
    !end do
    ...
    ! microphysics tendency of cloud liquid droplet number concentration due to deposition/condensation
    !micro_naai(:, 1:micro_nlev) = 0._kind_phys
    ! (ad-hoc) deposition/condensation nucleation in mixed-phase clouds (-37 C < T < 0 C) following Meyers et al. (1992)
    do i = 1, micro_ncol
      do k = 1, micro_nlev
        rk = micro_nlev - k + 1
        if ( tgrs(i,rk) .lt. 273.15_kind_phys .and. tgrs(i,rk) .gt. 236.15_kind_phys .and. qgrs(i,rk,ntcw) .gt. 1.e-12_kind_phys ) then
